...
namespace PowerpointGenerater.Powerpoint {
  ...
  /// interop klassen.
  class PowerpointWrapper : IDisposable {
    ...
    public delegate void GereedMelding(String foutmelding = null);
    private GereedMelding _setGereedmelding;

    public PowerpointWrapper(Voortgang voortgangDelegate, GereedMelding gereedmeldingDelegate) {
    ...
    public StatusMelding Initialiseer(IEnumerable<ILiturgieZoekresultaat> liturgie, string Voorganger, string Collecte1, string Collecte2, string Lezen,
      string Tekst, Instellingen instellingen) {
      lock (_locker) {
    ...
    private void PresentatieStatusWijzigingCallback(PowerpointGenerater.Powerpoint.PowerpointFunctions.Status nieuweStatus) {
